...
# SENTINEL 2
BITS_SENTINEL2 = {"10": {1: "cloud"}, "11": {1: "cirrus"}}
...
def generalMask(
    ...
    msg_eb = (
        "Band called 'mask' is for {} and was computed by geetools"
    ...
    def get_all_mask(encoded):
        ...
        all_masks = ee.Image(rest.iterate(func, initial)).select([0], [all_masks_name])
        mask = all_masks.Not()
        return mask
...
def modis13q1(
    options=("cloud", "adjacent", "shadow", "snow"),
    update_mask=True,
    add_mask_band=True,
    add_every_mask=False,
):
...
def landsat457SRPixelQA(
    options=("cloud", "shadow", "snow"),
    update_mask=True,
    add_mask_band=True,
    add_every_mask=False,
):
...
def landsat8SRPixelQA(
    options=("cloud", "shadow", "snow", "cirrus"),
    update_mask=True,
    add_mask_band=True,
    add_every_mask=False,
):
...
def landsatSR(
    options=("cloud", "shadow", "adjacent", "snow"),
    name="sr_mask",
    addBands=False,
    updateMask=True,
):
    ...
    """
    sr = {
        "bits": ee.Dictionary({"cloud": 1, "shadow": 2, "adjacent": 3, "snow": 4}),
        "band": "sr_cloud_qa",
    }

    pix = {
        "bits": ee.Dictionary({"cloud": 5, "shadow": 3, "snow": 4}),
        "band": "pixel_qa",
    }
